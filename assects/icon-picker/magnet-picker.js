const MagnetPicker = function (option) {
  ...
  var iconLibrary = allIcons;

  var selector = document.querySelector(defaultOpt.selector);
  if (null == selector) {
    return;
  }
  ...
  var magnetPickerWrap =
    "" +
    '<div class="aim-modal aim-open" id="aim-modal">' +
    '<div class="aim-modal--content">' +
    '<div class="aim-modal--header">' +
    '<div class="aim-modal--header-logo-area">' +
    '<span class="aim-modal--header-logo-title">Magnet Icon Picker</span>' +
    "</div>" +
    ...
    '<input name="" value="" placeholder="Filter by name...">' +
    ...
    '<button class="aim-insert-icon-button">Insert</button>' +
    "</div>" +
    "</div>" +
    "</div>";
  ...
  var magnetPickerDomEle = createDomEle(magnetPickerWrap);
  var sidebarTabs = magnetPickerDomEle.querySelector(
    ".aim-modal--sidebar-tabs"
  );
  var previewWrap = magnetPickerDomEle.querySelector(
    "#aim-modal--icon-preview"
  );
  var searchInput = magnetPickerDomEle.querySelector(
  ...
  document
    .querySelector(defaultOpt.onClick)
    .addEventListener("click", function (e) {
      if (null == selector.querySelector(".aim-modal")) {
        //push magnetPicker dom to selector
        selector.appendChild(magnetPickerDomEle);
      } else {
        //Icon library open if dom element exist
        magnetPickerDomEle.classList.remove("aim-close");
        magnetPickerDomEle.classList.add("aim-open");
      }
      document.querySelector(".aim-open .aim-modal--icon-search input").focus();
      //Icon library close by clicking close button
      magnetPickerDomEle
        .querySelector(".aim-modal--header-close-btn")
        .addEventListener("click", function (e) {
          magnetPickerDomEle.classList.add("aim-close");
          magnetPickerDomEle.classList.remove("aim-open");
        });
      ...
      document.querySelectorAll(".aim-icon-item").forEach(function (item, key) {
        item.addEventListener("click", function (e) {
          iconWrap.forEach(function (item, key) {
            item.classList.remove("magnetPicker-selected");
          });
          e.currentTarget.classList.toggle("magnetPicker-selected");
          // e.currentTarget.classList.add('magnetPicker-selected');
        });
      });

      //Insert button
      magnetPickerDomEle
        .querySelector(".aim-insert-icon-button")
        .addEventListener("click", function (e) {
          e.preventDefault();
          var selected = document.querySelector(".magnetPicker-selected");
          if (null !== selected) {
            var selectedClass = selected.querySelector("i").classList.value;
            document.querySelector(option.inputField).value = selectedClass;
            var wrap = document.querySelector(option.iconPlaceholder);
  ...
  document
    .querySelector(defaultOpt.removeIcon)
    .addEventListener("click", function (e) {
      document.querySelector(option.iconPlaceholder + " i").classList.value =
        "fas fa-circle";
      document.querySelector(option.inputField).value = "";
    });
  ...
  function searchFunc(e) {
    ...
    searchFilterFunc(iconWrap, searchText, "filter");
  }
  ...
  function searchFilterFunc(filterItems, filterText, dataName) {
    filterIcon = Object.entries(filterItems).filter(function (item, index) {
      return -1 !== item[1].dataset[dataName].indexOf(filterText);
    });
  ...
  function sidebarFilterFunc(filterItems, filterText) {
    filterIcon = Object.entries(filterItems).filter(function (item, index) {
      return (
        "all" === filterText || filterText === item[1].dataset["libraryId"]
      );
    });
    iconItemPush(filterIcon);
  }
  ...
  function clickHandlerFunc(e) {
    if (!e.currentTarget.classList.contains("magnetPicker-active")) {
      sideBarBtn.forEach(function (item, key) {
        item.classList.remove("magnetPicker-active");
      });
      e.currentTarget.classList.add("magnetPicker-active");
    }
  ...
  function setSideBarList(library, object) {
    object.forEach(function (item, index) {
      var listItem = {
        ...
        "list-icon": item[1]["list-icon"].length
          ? item[1]["list-icon"]
          : "far fa-dot-circle",
        "library-id": item[1]["icon-style"].length
          ? item[1]["icon-style"]
          : "all",
      };
  ...
  function sideBarListMarkup(sideBarList) {
    ...
    sideBarList.forEach(function (item, index) {
      if ("all" !== item["library-id"]) {
        markup +=
          '<div class="aim-modal--sidebar-tab-item" data-library-id="' +
          item["library-id"] +
          '"><i class="' +
          item["list-icon"] +
          '"></i>' +
        ...
        markup +=
          '<div class="aim-modal--sidebar-tab-item magnetPicker-active" data-library-id="' +
          item["library-id"] +
          '"><i class="' +
          item["list-icon"] +
          '"></i>' +
  ...
  function iconItemMarkup(libraryItem) {
    ...
    libraryItem["icons"].forEach(function (item, index) {
      markup +=
        '<div class="aim-icon-item" data-library-id="' +
        library +
        '" data-filter="' +
        item.replace(prefix, "") +
        '"><div class="aim-icon-item-inner"><i class="' +
        item +
        '"></i><div class="aim-icon-item-name" title="' +
        item.replace(prefix, "") +
        '">' +
  ...
  function setIconAndSidebarList(iconLibrary) {
    Object.entries(iconLibrary).forEach(function (item, index) {
